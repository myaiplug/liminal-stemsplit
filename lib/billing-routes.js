...
import { isProProduct } from './vst-products.js';
import {
  hashPayload,
  listLicensesSafe,
...
function detectShopifyProduct(order) {
  ...
  for (const item of items) {
    ...
    if (hay.includes('screwai') || hay.includes('screw')) {
      return { isMatch: true, product: 'screwai_pro', productName: item.title || 'ScrewAI PRO' };
    }
  ...
  const needle = (process.env.SHOPIFY_PRODUCT_NEEDLE || 'stemsplit|liminal|pro|coproducer|degloss|screwai|screw').toLowerCase();
...
export function createBillingRouter() {
  ...
  router.post('/webhooks/stripe', async (req, res) => {
    ...
    const plan = session.metadata?.plan || (isProProduct(product) ? 'pro' : 'vst');
    ...
    try {
      ...
      const emailResult = await deliverActivationEmailAfterPurchase({
        ...
        emailOpts: { source: 'stripe', secondaryKey: keys.secondary, product },
      });
      res.json({
        ok: true,
  ...
  router.post('/webhooks/gumroad', async (req, res) => {
    ...
    const rawProductName = String(body.product_name || body['product[name]'] || '').toLowerCase();
    const gumroadProduct = rawProductName.includes('screwai')
      ? 'screwai_pro'
      : rawProductName.includes('coproducer')
        ? 'coproducer_pro'
        : rawProductName.includes('degloss') || rawProductName.includes('reverb')
          ? 'vst_reverb_degloss'
          : 'stemsplit_pro';
    ...
    try {
      const saved = upsertLicense({
        email,
        source: 'gumroad',
        plan: body.plan || (isProProduct(gumroadProduct) ? 'pro' : 'vst'),
