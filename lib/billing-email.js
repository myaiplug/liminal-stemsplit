...
function sendNativeGmailSmtp({ user, pass, to, subject, html }) {
  return new Promise((resolve) => {
    ...
    socket.on('data', (data) => {
      ...
      if (lastLine.startsWith('220') && step === 0) {
        ...
        socket.write('QUIT\r\n');
        socket.end();
        resolve({ sent: true, message: 'Message accepted for delivery' });
      } else if (lastLine.startsWith('5') || lastLine.startsWith('4')) {
        socket.end();
        resolve({ sent: false, reason: `SMTP error: ${lastLine}` });
      }
    });

    socket.on('error', (err) => {
      resolve({ sent: false, reason: `Socket error: ${err.message}` });
    });
  });
}
...
const SCREWAI_SITE_URL = process.env.SCREWAI_SITE_URL || 'https://myaiplug.github.io/ScrewAI-App/';
const SCREWAI_DOWNLOAD_URL = process.env.SCREWAI_DOWNLOAD_URL || 'https://myaiplug.github.io/ScrewAI-App/';
const SCREWAI_FROM_EMAIL = process.env.SCREWAI_FROM_EMAIL || 'ScrewAI PRO <onboarding@myaiplug.com>';
...
function buildActivationHtml(email, accessPassword, opts = {}) {
  ...
  const isScrewAI = product === 'screwai_pro' || (opts.productName || '').toLowerCase().includes('screwai');
  ...
  const brandHeader = isScrewAI ? 'ScrewAI PRO' : isCoProducer ? 'NoDAW CoProducer PRO' : isDeGloss ? 'ReVerb-DeGloss VST' : 'Liminal StemSplit Pro';
  const licenseTitle = isScrewAI ? 'Your ScrewAI PRO license is ready' : isCoProducer ? 'Your CoProducer PRO license is ready' : isDeGloss ? 'Your ReVerb-DeGloss license is ready' : 'Your Pro license is ready';

  const stepsHtml = isScrewAI
    ? `<strong style="color:#e2e8f0">Activate in 2 steps</strong><br>
       1) Download the Windows installer<br>
       2) Open <strong style="color:#a7f3d0">Unlock PRO</strong> and paste your purchase email + license key`
    : isCoProducer
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#0f172a;border:1px solid rgba(16,185,129,0.2);border-radius:16px;overflow:hidden">
        ...
        <tr><td style="padding:8px 36px 28px;line-height:1.7;font-size:15px;color:#94a3b8">
          ...
          <p style="margin:20px 0 0;text-align:center">
            <a href="${isScrewAI ? SCREWAI_DOWNLOAD_URL : DOWNLOAD_URL}" style="display:inline-block;padding:12px 24px;background:linear-gradient(135deg,#059669,#10b981);color:#fff;text-decoration:none;border-radius:999px;font-weight:600;font-size:14px;margin-right:8px">Download installer →</a>
            <a href="${isScrewAI ? SCREWAI_SITE_URL : SITE_URL}" style="display:inline-block;padding:12px 24px;border:1px solid rgba(16,185,129,0.4);color:#6ee7b7;text-decoration:none;border-radius:999px;font-weight:600;font-size:14px">Website →</a>
          </p>
        </td></tr>
      </table>
    </td></tr>
...
export async function sendProActivationEmail(email, accessPassword, opts = {}) {
  ...
  if (!gmailPass && !apiKey) {
    return { sent: false, reason: 'Neither GMAIL_APP_PASSWORD nor RESEND_API_KEY is set' };
  }
  ...
  const isCoProducer = product === 'coproducer_pro' || (opts.productName || '').toLowerCase().includes('coproducer');
  const isDeGloss = product === 'vst_reverb_degloss' || (opts.productName || '').toLowerCase().includes('degloss');
  const subject = isScrewAI
    ? 'Your ScrewAI PRO license'
    : isCoProducer
  ...
  const html = buildActivationHtml(email, accessPassword, opts);

  if (gmailPass) {
    const res = await sendNativeGmailSmtp({ user: gmailUser, pass: gmailPass, to: email, subject, html });
    if (res.sent) {
      return { sent: true, message: res.message, provider: 'gmail_smtp' };
    }
    console.error('[Gmail SMTP Error]:', res.reason);
    if (!apiKey) return { sent: false, reason: `Gmail SMTP failed: ${res.reason}` };
  }
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: isScrewAI ? SCREWAI_FROM_EMAIL : FROM_EMAIL,
      to: [email],
      subject,
      html,
    }),
