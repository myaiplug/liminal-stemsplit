    ...
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#0f172a;border:1px solid rgba(16,185,129,0.2);border-radius:16px;overflow:hidden">
        <tr><td style="padding:32px 36px 20px;text-align:center">
          <div style="font-size:11px;letter-spacing:0.15em;text-transform:uppercase;color:#34d399;margin-bottom:12px">Liminal StemSplit Pro</div>
          <h1 style="margin:0;font-size:24px;color:#fff;font-weight:800">Your Pro license is ready</h1>
        ...
        <tr><td style="padding:8px 36px 28px;line-height:1.7;font-size:15px;color:#94a3b8">
          <p style="margin:0 0 16px">Thanks for purchasing. Use the details below to activate inside the desktop app (Identity Console → Activate Pro).</p>
          ...
          <p style="margin:20px 0 0;font-size:12px;color:#64748b">Tip: Use the file named <code style="color:#94a3b8">NoDAW-Liminal-StemSplit-v0.5.0-Setup-WORKING.exe</code> if listed — single file, most reliable.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
export async function sendProActivationEmail(email, accessPassword, opts = {}) {
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: FROM_EMAIL,
      to: [email],
      subject: 'Your Liminal StemSplit Pro license',
