...
function updateJob(jobId, patch) {
  ...
  queue.jobs[index] = { ...queue.jobs[index], ...patch, updatedAt: new Date().toISOString() };
  saveQueue(queue);
...
export async function deliverActivationEmailAfterPurchase({
  ...
  if (!raced.sent) void deliveryPromise;
...
export async function dispatchActivationEmailForEmail(email) {
  ...
  if (!job) return { sent: false, reason: 'No pending activation email for this address', queued: false };
