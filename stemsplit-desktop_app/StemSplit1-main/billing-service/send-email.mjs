  ...
  if (gmailPass) {
    try {
      ...
      const info = await transporter.sendMail({
        from: `Liminal StemSplit <${gmailUser}>`,
        to: email,
        subject,
        html,
      });
      return { sent: true, id: info.messageId, provider: 'gmail_smtp' };
    } catch (err) {
      console.error('[Gmail SMTP Error]:', err.message);
      if (!apiKey) return { sent: false, reason: `Gmail SMTP failed: ${err.message}` };
    }
  }
