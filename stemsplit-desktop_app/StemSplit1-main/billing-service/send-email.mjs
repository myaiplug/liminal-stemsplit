      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#0f172a;border:1px solid rgba(16,185,129,0.2);border-radius:16px;overflow:hidden">
        ...
        <tr><td style="padding:8px 36px 28px;line-height:1.7;font-size:15px;color:#94a3b8">
          ...
          <p style="margin:20px 0 0;text-align:center">
            <a href="https://github.com/myaiplug/StemSplit1/releases/latest" style="display:inline-block;padding:12px 24px;background:linear-gradient(135deg,#059669,#10b981);color:#fff;text-decoration:none;border-radius:999px;font-weight:600;font-size:14px">Download app →</a>
          </p>
        </td></tr>
      </table>
    </td></tr>
...
export async function sendProActivationEmail(email, accessPassword) {
  ...
  if (!email || !accessPassword) {
    return { sent: false, reason: 'Missing email or access password' };
  }
  ...
  if (!response.ok) {
    return { sent: false, reason: data.message || response.statusText };
  }
