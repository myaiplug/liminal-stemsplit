...
const ReactorZone: React.FC = () => {
    ...
    const stemLabels = useMemo(() => {
        if (splitEngine === 'instrument') {
            return getInstrumentStemLabels(splitModelVariant);
        }
        if (splitEngine === 'karaoke') return ['lead', 'backing', 'instrumental'];
        if (splitEngine === 'drumsep') {
            if (splitStems === '7' || splitStems === '6' || splitModelVariant.startsWith('drumsep_mdx23c_'))
                return ['kick', 'snare', 'toms', 'hh', 'ride', 'crash'];
            return ['kick', 'snare', 'toms', 'cymbals'];
        }
    ...
    }, [status, isProcessingQueue, queueIndex, pendingFiles, startSeparation, splitOptions, play]);
        ...
        (async () => {
            ...
            unlisten = await getCurrentWindow().onDragDropEvent((event) => {
                ...
                if (payload.type === 'drop') {
                    ...
                    setUiError(null);
                    applySelectedPaths(payload.paths);
                }
            });
        })();
    ...
    const executePendingSplit = useCallback(async () => {
        ...
        if (pendingFilePath) {
            ...
            if (pendingFiles.length > 1) {
                ...
                setQueueIndex(0);
                startSeparation(pendingFiles[0], splitOptions);
            } else {
                // SINGLE FILE (Classic)
    ...
    return (
        <div className="relative min-h-screen flex flex-col items-center justify-center bg-slate-950 overflow-x-hidden overflow-y-auto">
                ...
                {showSettings && (
                    ...
                    >
                        <motion.div 
                            ...
                            className="bg-slate-900 border border-cyan-900/50 p-5 rounded-xl w-full max-w-3xl max-h-[92vh] overflow-y-auto custom-scrollbar shadow-2xl shadow-cyan-900/20"
                            onClick={(e) => e.stopPropagation()}
                        >
                            <h2 className="text-cyan-400 font-mono text-lg mb-3 border-b border-slate-800 pb-2 sticky top-0 bg-slate-900 z-10">Separation Configuration</h2>
                            ...
                            <div className="space-y-3 font-mono text-sm text-slate-300">
                                ...
                                <div>
                                    ...
                                    {splitEngine === 'instrument' && (
                                        <span className="text-[10px] text-lime-300 mt-1 block">
                                            Instrument mode isolates a target stem + other. Best workflow: 4-stem first, then re-run on guitar/piano/other.
                                        </span>
                                    )}
                                    {splitModelVariant === 'postfx_matchering' && (
                                        <div className="mt-2 space-y-1">
                                            <label className="text-[10px] text-slate-400 block">Matchering reference track</label>
                                            <div className="flex gap-2">
                                                <input
                                                    readOnly
                                                    value={matcheringReference?.split(/[\\/]/).pop() || 'No reference selected'}
                                                    className="flex-1 bg-slate-950 border border-slate-700 rounded px-2 py-1 text-[10px] text-slate-300"
                                                />
                                                <button
                                                    type="button"
                                                    className="text-[10px] px-2 py-1 rounded bg-amber-900/40 border border-amber-700/50 text-amber-200"
                                                    onClick={async () => {
                                                        if (!isTauriRuntime()) return;
                                                        const picked = await dialogOpen({ multiple: false, filters: [{ name: 'Audio', extensions: ['wav', 'mp3', 'flac'] }] });
                                                        if (picked && typeof picked === 'string') setMatcheringReference(picked);
                                                    }}
                                                >
                                                    Browse
                                                </button>
            ...
            <AnimatePresence>
                {loadedFilePath && (
                    <OriginalPlayer
                        key={`${loadedFilePath}-${sourceTitle ?? 'source'}`}
                        filePath={loadedFilePath}
                        displayTitle={sourceTitle ?? undefined}
