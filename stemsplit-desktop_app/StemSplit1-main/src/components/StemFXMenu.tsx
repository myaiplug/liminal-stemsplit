...
import { USER_VSTS as DEFAULT_USER_VSTS } from '@/lib/user-vsts';
...
const USER_VSTS: VSTPlugin[] = DEFAULT_USER_VSTS.map((vst) => ({
  ...vst,
  enabled: vst.enabled ?? true,
}));
...
const StemFXMenu: React.FC<StemFXMenuProps> = ({ stemType, stemFilePath, isOpen, onClose, onApply, isFreeMode = false }) => {
    ...
    const handleApplyFX = useCallback(async (isPreview = false, resumeTime = -1) => {
        ...
        try {
            ...
            if (!isPro && enabledModules.length > 0) {
                setStatusMsg('Built-in FX modules require Pro. VST plugins have a separate free trial.');
                setIsApplying(false);
                return;
            }
            ...
            if (!isPro) {
                for (const vst of enabledVSTs) {
                    ...
                    const access = await ensureVstAccess(vst.id, 'apply');
                    if (!access.allowed) {
                        setUpgradePluginId(vst.id);
                        setStatusMsg(access.reason || 'Trial apply limit reached.');
                        setIsApplying(false);
                        return;
                    }
                }
    ...
    }, [activeModules, loadedVSTs, stemFilePath, livePreview, onApply, isPro, refreshEntitlements, cleanupAllAudio]);
                                    ...
                                    <div className="space-y-2">
                                        {loadedVSTs.map(vst => {
                                            ...
                                            <div key={vst.id} className={`flex items-center justify-between p-2 rounded border transition ${vst.enabled ? 'bg-slate-800 border-blue-500/30' : 'bg-slate-900/50 border-slate-700 opacity-70'}`}>
                                                ...
                                                {!DEFAULT_USER_VSTS.some(u => u.id === vst.id) && (
                                                    <button 
