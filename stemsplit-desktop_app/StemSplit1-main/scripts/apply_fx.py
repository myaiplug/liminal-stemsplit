...
def resolve_vst_path(path: str) -> str:
    """Resolve a VST3 bundle folder (or alias) to the loadable plugin binary."""
    if not path:
        return path
    ...
    if normalized.lower().endswith(".vst3") and os.path.isdir(normalized):
        arch_dirs = [
            os.path.join(normalized, "Contents", "x86_64-win"),
            os.path.join(normalized, "Contents", "x86-win"),
            os.path.join(normalized, "Contents", "Win64"),
            os.path.join(normalized, "Contents", "MacOS"),
        ]
        for arch_dir in arch_dirs:
            if not os.path.isdir(arch_dir):
                continue
            for name in sorted(os.listdir(arch_dir)):
                if name.lower().endswith((".vst3", ".dll")):
...
def apply_fx_chain(audio, sample_rate, fx_config):
    ...
    if is_legacy:
        ...
        for vst_entry in vsts:
            ...
            resolved_vst_path = resolve_vst_path(vst_path)
            if resolved_vst_path and os.path.exists(resolved_vst_path):
                try:
                    if resolved_vst_path != vst_path:
                        logger.info(f"Resolved VST path: {vst_path} -> {resolved_vst_path}")
                    logger.info(f"Loading VST: {resolved_vst_path}")
                    plugin = load_plugin(resolved_vst_path)
                    if vst_state:
                         try:
