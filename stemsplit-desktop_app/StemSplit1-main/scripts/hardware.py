...
def _probe_nvidia_smi() -> Tuple[Optional[str], float]:
    """
    Query NVIDIA driver directly. Works even when PyTorch is CPU-only,
    but we only select CUDA when torch.cuda.is_available() is true.
    """
    try:
        ...
        # nounits returns MiB on Windows/Linux.
        vram_gb = memory_value / 1024.0
...
def get_processing_config() -> Dict[str, Any]:
    ...
    try:
        ...
        if cuda_device == 'cuda':
            ...
            # No CUDA from PyTorch — check if hardware GPU exists but runtime is CPU-only.
            mkl_threads = _configure_mkl_threads(cpu_cores)
            config["device"] = "cpu"
            config["split_strategy"] = "default"
            config["mkl_threads"] = mkl_threads

            gpu_name, gpu_vram = _probe_nvidia_smi()
            if gpu_name:
                config["gpu_vram"] = gpu_vram
                config["status"] = (
                    f"NVIDIA GPU detected ({gpu_name}, {gpu_vram:.2f}GB) but PyTorch CUDA is unavailable. "
                    f"Using CPU with {mkl_threads} threads. Run Python setup repair to install CUDA PyTorch."
                )
                logger.warning(config["status"])
            else:
                config["status"] = (
                    f"No CUDA device detected. Using CPU with {mkl_threads} MKL threads (75% of {cpu_cores} cores)."
                )
