param(
    ...
    [string]$InstallDir = "$Env:ProgramFiles\NoDAW Liminal",
...
function Resolve-InstallerPath {
    ...
    $candidate = Get-ChildItem "installers" -Filter "Liminal-StemSplit-Setup-*-Windows-x64-Online.exe" -ErrorAction SilentlyContinue |
    ...
    if (-not $candidate) {
        throw "No installer found. Expected 'installers\\Liminal-StemSplit-Setup-*-Windows-x64-Online.exe'."
    }
...
$mainExeCandidates = @(
    (Join-Path $InstallDir "Liminal™.exe"),
